...
let User = function(data, getAvatar) {
    //the first data is not the next data. we're storing the second data to the first data.
    this.data = data;
    this.errors = [];
...
User.prototype.validate = function() {
    return new Promise(async (resolve, reject) => {
        ...
        if(this.data.password.length > 0 && this.data.password.length < 12) {this.errors.push('Password must be atleast 12 character')}
        ...
        //Only if username is valid then cheack to see if it's already taken
    ...
    //resolve and reject can be anything, but those are industry standards
